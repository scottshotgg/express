...
// SubOperands returns the addition of two operands based on their type
func (p *Parser) SubOperands(left, right token.Value) (token.Value, error) {
	...
	if leftType == rightType {
		...
		case token.VarType:
			...
			var err error
			valueToken, err = p.SubOperands(left, right)
			if err != nil {
		...
		case token.ObjectType:
			...
			for key, value1 := range left.True.(map[string]token.Value) {
				if value2, ok := result[key]; ok {
					resultValue, err := p.SubOperands(value1, value2)
		...
		default:
			fmt.Println("Type not declared for SubOperands", left, right, leftType, rightType)
			os.Exit(9)
		}
	...
	err := errors.New("Could not perform SubOperands on operands")
